...
/* ── TOKENS — Ivory + ink + terracotta (4 colors total) ── */
:root {
  --sand: #efe7d6;        /* ivory page bg */
  --sand-mid: #e8dfc8;    /* slightly darker surface */
  --sand-deep: #b46b50;   /* legacy alias -> accent */
  --bark: #1a1a18;        /* ink (text + dark surface) */
  --bark-mid: #6b6056;    /* muted secondary text */
  --bark-light: #6b6056;
  --warm-accent: #b46b50; /* single warm accent — terracotta */
  --teal: #b46b50;        /* legacy alias -> accent */
  --teal-mid: #b46b50;
...
.ct-nav {
  ...
  /* Align inner content to the same 1180px line as every section below.
     Full-bleed bar (for background blur), but content stays within the
     same horizontal gutter as the rest of the page. */
...
/* On viewports narrower than 1180 + small gutter, use a fixed 24px gutter. */
...
.ct-nav__center {
  width: 100px; /* same visual balance as CTA side */
}
...
/* ══ §2 HERO — Luxury edition ══ */
.ct-hero {
  --px: 0; --py: 0;
  position: relative;
  overflow: hidden;
  background:
    radial-gradient(120% 80% at 80% -10%, #ece1d2 0%, transparent 55%),
...
.ct-hero__glow--a {
  ...
  background: radial-gradient(circle, rgba(214,184,150,.55) 0%, rgba(214,184,150,0) 65%);
  transform: translate3d(calc(var(--px) * -22px), calc(var(--py) * -22px), 0);
}
.ct-hero__glow--b {
  bottom: -220px; left: -180px;
  width: 640px; height: 640px;
  background: radial-gradient(circle, rgba(36,61,56,.18) 0%, rgba(36,61,56,0) 60%);
  transform: translate3d(calc(var(--px) * 18px), calc(var(--py) * 18px), 0);
}

/* faint vertical grid lines (editorial) */
.ct-hero__grid-lines {
  position: absolute; inset: 120px 8% 120px 8%;
  display: grid; grid-template-columns: repeat(5, 1fr);
  pointer-events: none; z-index: 2; opacity: .35;
}
.ct-hero__grid-lines span {
  border-left: 1px solid rgba(31,31,31,.06);
}
...
.ct-hero__strip-track {
  display: flex; gap: 40px;
  animation: ct-strip-scroll 38s linear infinite;
  white-space: nowrap;
}
.ct-hero__strip-group {
...
.ct-reveal {
  ...
  filter: blur(6px);
  transition: opacity 1.1s cubic-bezier(.2,.7,.2,1) var(--d, 0s),
              transform 1.1s cubic-bezier(.2,.7,.2,1) var(--d, 0s),
              filter 1.1s ease var(--d, 0s);
}
.ct-hero--in .ct-reveal {
...
.ct-hero__h1-underline {
  ...
  transition: stroke-dashoffset 1.6s cubic-bezier(.2,.7,.2,1) .9s;
}
...
.ct-hero__avail {
  ...
  border: 1px solid rgba(58,87,79,.25);
  border-radius: 100px;
  background: rgba(238,247,243,.5);
}
.ct-hero__avail-dot {
  width: 8px; height: 8px; border-radius: 50%;
  background: #4a8a78;
  box-shadow: 0 0 0 0 rgba(74,138,120,.6);
  animation: ct-avail-pulse 2.2s ease-out infinite;
}
@keyframes ct-avail-pulse {
  0% { box-shadow: 0 0 0 0 rgba(74,138,120,.55); }
  70% { box-shadow: 0 0 0 12px rgba(74,138,120,0); }
  100% { box-shadow: 0 0 0 0 rgba(74,138,120,0); }
}
...
.ct-hero__btn-primary {
  ...
  cursor: pointer;
  z-index: 1;
  transition: color .4s ease, transform .35s ease, border-color .4s ease;
}
.ct-hero__btn-primary::before {
  content: '';
  position: absolute; inset: 0;
  background: var(--bark);
...
.ct-hero__btn-ghost {
  ...
  color: #75695f; background: none; border: none; cursor: pointer;
...
.ct-hero__stat {
  flex: 1;
  padding: 0 28px;
  border-left: 1px solid rgba(194,169,146,.4);
}
...
.ct-hero__right {
  ...
  transition: transform .8s cubic-bezier(.2,.7,.2,1);
  padding-left: 56px; /* room for vertical ruler */
}
...
.ct-hero__rule-line {
  flex: 1; width: 1px;
  background: linear-gradient(180deg, transparent 0%, rgba(31,31,31,.25) 30%, rgba(31,31,31,.25) 70%, transparent 100%);
}

/* Stage that contains the portrait + overlays */
.ct-hero__stage {
  position: relative;
  width: 100%;
  height: 100%;
...
.ct-hero__ring {
  ...
  border: 1px dashed rgba(194,169,146,.55);
  border-radius: 50%;
  animation: ct-ring-spin 60s linear infinite;
  pointer-events: none;
  z-index: 1;
}
.ct-hero__ring::after {
  ...
  position: absolute;
  inset: 24px;
  border: 1px solid rgba(194,169,146,.3);
  border-radius: 50%;
  animation: ct-ring-breathe 6s ease-in-out infinite;
}
@keyframes ct-ring-spin { to { transform: translateX(-50%) rotate(360deg); } }
@keyframes ct-ring-breathe {
...
.ct-hero__frame {
  ...
  background:
    linear-gradient(180deg, #efe4d2 0%, #d9c6ad 100%);
  overflow: hidden;
  box-shadow:
    inset 0 0 0 1px rgba(255,255,255,.45),
    inset 0 -60px 80px -40px rgba(80,60,40,.18),
    0 60px 80px -40px rgba(80,60,40,.25);
}
...
.ct-hero__frame-grain {
  position: absolute; inset: 0;
  background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='g'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='3'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23g)' opacity='0.5'/%3E%3C/svg%3E");
...
.ct-hero__frame::after {
  /* warm bottom vignette so the caption is readable */
  content: '';
  position: absolute;
  left: 0; right: 0; bottom: 0;
  height: 38%;
  background: linear-gradient(180deg, transparent 0%, rgba(31,27,22,.55) 100%);
  z-index: 3;
  pointer-events: none;
}
.ct-hero__frame-caption {
  position: absolute;
...
/* Seal — anchored to top-right of the frame, never clipped */
.ct-hero__seal {
  position: absolute;
  top: 6%;
  right: 4%;
...
.ct-hero__seal svg {
  ...
  animation: ct-seal-spin 28s linear infinite;
}
.ct-hero__seal-text {
  font-family: var(--sans);
  font-size: 8px;
...
/* Quote card — overlapping bottom-left of frame */
.ct-hero__card {
  position: absolute;
  margin-top: 30px;
  left: 28px;
  ...
  box-shadow: 0 30px 50px -20px rgba(80,60,40,.25);
  animation: ct-card-float 7s ease-in-out infinite;
}
...
/* Accolade — moved to TOP, away from scroll-to-top button */
.ct-hero__accolade {
  position: absolute;
  top: 36px;
  right: 12px;
  ...
  animation: ct-card-float 8s ease-in-out -2s infinite;
}
.ct-hero__accolade-eyebrow {
  display: block;
  font-family: var(--sans);
  font-size: 8.5px;
  letter-spacing: .32em;
...
.ct-hero__scroll-line {
  ...
  background: rgba(154,135,118,.3);
  overflow: hidden;
}
.ct-hero__scroll-line span {
  position: absolute;
  top: -40%; left: 0;
  width: 100%; height: 40%;
  background: var(--bark);
  animation: ct-scroll-drop 2.4s cubic-bezier(.7,0,.3,1) infinite;
}
@keyframes ct-scroll-drop {
  0% { top: -40%; }
  60% { top: 100%; }
  100% { top: 100%; }
}
...
@media (max-width: 1023px) {
  .ct-hero__inner { padding: 40px 28px 100px; min-height: auto; }
  .ct-hero__strip { top: 80px; }
  .ct-hero__bracket { width: 36px; height: 36px; }
  .ct-hero__press { padding: 0 28px 28px; flex-direction: column; align-items: flex-start; gap: 18px; }
  .ct-hero__press-row { width: 100%; gap: 24px; }
  .ct-hero__stats { gap: 0; flex-wrap: wrap; }
}
...
.ct-photo-inner {
  ...
  width: 100%;
  aspect-ratio: 4/5;
  background: linear-gradient(160deg, #e4d8ca 0%, #cfc0ac 100%);
  border-radius: 3px;
  display: flex;
  align-items: flex-end;
  overflow: hidden;
...
.ct-about__blockquote {
  margin: 28px 0 48px;
  padding-left: 20px;
  border-left: 2px solid #c9b99f;
}
.ct-about__blockquote-text {
  font-family: var(--serif);
  font-size: 15px;
  line-height: 1.7;
  color: #8c7c6d;
...
.ct-about__strip-link:hover { color: var(--teal); border-color: var(--teal); }

/* ══ §3.5 SERVICES (editorial) ══ */
.ct-svc {
  position: relative;
  overflow: hidden;
  background: #f7f1e7;
...
.ct-svc__bg {
  ...
  inset: 0;
  background:
    radial-gradient(80% 50% at 90% 0%, rgba(214,184,150,.25) 0%, transparent 60%),
    radial-gradient(70% 60% at 0% 100%, rgba(36,61,56,.08) 0%, transparent 60%);
  z-index: 0;
  pointer-events: none;
}
.ct-svc__bg-line {
  ...
  width: 1px;
  background: linear-gradient(180deg, transparent 0%, rgba(31,31,31,.06) 18%, rgba(31,31,31,.06) 82%, transparent 100%);
  z-index: 0;
}
.ct-svc__inner {
  position: relative;
...
.ct-svc__carousel::before,
.ct-svc__carousel::after {
  content: '';
  position: absolute;
  top: 0; bottom: 28px;
  width: 80px;
...
.ct-svc__carousel::before {
  left: 0;
  background: linear-gradient(90deg, #f7f1e7 0%, rgba(247,241,231,0) 100%);
}
.ct-svc__carousel::after {
  right: 0;
  background: linear-gradient(270deg, #f7f1e7 0%, rgba(247,241,231,0) 100%);
}
...
.ct-svc__arrow {
  ...
  box-shadow: 0 18px 30px -16px rgba(80,60,40,.18);
  transition: background .3s ease, color .3s ease, transform .3s ease, opacity .3s ease, border-color .3s ease;
}
.ct-svc__arrow--left  { left: -28px; }
.ct-svc__arrow--right { right: -28px; }
.ct-svc__arrow:hover:not(.ct-svc__arrow--disabled) {
  background: var(--bark);
  color: #f5efe8;
  border-color: var(--bark);
  transform: translateY(calc(-50% - 14px)) scale(1.04);
}
.ct-svc__arrow--left:hover:not(.ct-svc__arrow--disabled) {
  transform: translateY(calc(-50% - 14px)) translateX(-3px) scale(1.04);
}
.ct-svc__arrow--right:hover:not(.ct-svc__arrow--disabled) {
  transform: translateY(calc(-50% - 14px)) translateX(3px) scale(1.04);
}
.ct-svc__arrow--disabled {
  opacity: .35;
  cursor: default;
}
...
.ct-svc-card {
  ...
  border-radius: 28px;
  background:
    linear-gradient(180deg, rgba(255,253,248,.95) 0%, rgba(247,241,231,.85) 100%);
  padding: 32px 30px 28px;
  box-shadow:
    inset 0 0 0 1px rgba(255,255,255,.5),
    0 30px 50px -30px rgba(80,60,40,.18);
...
.ct-svc-card::before {
  /* curved inner highlight stroke */
  content: '';
  position: absolute;
  inset: 6px;
  border: 1px dashed rgba(194,160,122,.35);
  border-radius: 22px;
  pointer-events: none;
  z-index: 0;
}
.ct-svc-card::after {
  /* warm soft glow on hover */
  content: '';
  position: absolute;
  inset: -1px;
  border-radius: 28px;
  background: radial-gradient(70% 50% at 50% 0%, rgba(194,160,122,.18) 0%, transparent 70%);
  opacity: 0;
  z-index: -1;
...
.ct-svc-card:hover {
  ...
  border-color: rgba(194,160,122,.55);
  box-shadow:
    inset 0 0 0 1px rgba(255,255,255,.6),
    0 40px 60px -30px rgba(80,60,40,.28);
}
...
.ct-svc-card:hover .ct-svc-card__tag {
  border-color: rgba(194,160,122,.5);
...
.ct-svc__hint-line {
  ...
  background: rgba(194,160,122,.55);
}
...
@media (max-width: 1023px) {
  .ct-svc { padding: 100px 28px 120px; }
  .ct-svc__head {
    grid-template-columns: 1fr;
    gap: 32px;
    padding-bottom: 48px;
  }
  .ct-svc-card { flex: 0 0 86vw; max-width: 360px; padding: 26px 22px 22px; }
  .ct-svc-card__meta { gap: 12px; }
  .ct-svc__arrow { width: 44px; height: 44px; }
  .ct-svc__arrow--left  { left: -8px; }
  .ct-svc__arrow--right { right: -8px; }
  .ct-svc__carousel::before,
  .ct-svc__carousel::after { width: 36px; }
  .ct-svc__bg-line { display: none; }
}
...
.ct-services__blob {
  ...
  border-radius: 50%;
  background: rgba(215,197,178,.5);
  filter: blur(60px);
  pointer-events: none;
}
.ct-services__inner { position: relative; z-index: 10; max-width: 1280px; margin: 0 auto; }
.ct-services__header {
  display: flex;
  justify-content: space-between;
  align-items: flex-end;
...
.ct-service-card:hover { background: rgba(181,156,132,.08); }
...
.ct-service-card:hover .ct-service-card__num { color: #cbb7a1; }
.ct-service-card__accent {
...
.ct-service-card:hover .ct-service-card__arrow { transform: translateX(5px); color: var(--warm-accent); }
...
/* Stats slide */
.ct-slide-stats {}
.ct-slide-stats__headline {
  font-family: var(--serif);
  font-size: clamp(2rem,3vw,3rem);
  font-weight: 400;
  margin: 0 0 36px;
}
.ct-slide-stats__grid {
  display: grid;
...
/* Specialties slide */
.ct-slide-specialties {}
.ct-slide-specialties__headline {
  font-family: var(--serif);
  font-size: clamp(2rem,3vw,3rem);
  font-weight: 400;
  margin: 0 0 32px;
...
.ct-date-card--active { border-color: #7f8d88; background: #7f8d88; color: white; }
...
.ct-slot--active { border-color: #7f8d88; background: #7f8d88; color: white; }
...
.ct-field:focus { border-color: #9a8774; background: rgba(255,255,255,.35); }
...
.ct-booking__cta--active:hover { transform: translateY(-2px); background: var(--teal); }
...
.ct-footer__inner {
  ...
  gap: 60px;
  padding-bottom: 60px;
  border-bottom: 1px solid rgba(181,156,132,.35);
}
